...
class _AreaDataSheetState extends State<AreaDataSheet> {
  ...
  void initState() {
    ...
    _areaBloc.state.selectedCustomerIndex = 0; // Default to 0, but validate later
    _areaBloc.state.selectedContainerIndex = 0; // Default to 0, but validate later
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Customs.DataSheet(
      ...
      children: [
        BlocListener<ContainerInteractionBloc, ContainerInteractionState>(
          listener: (context, state) {
            if (state.getAddContainerStatus == AddContainerStatus.success) {
              if (_areaBloc.state.customers![_areaBloc.state.selectedCustomerIndex!].containers!.length == 1 && widget.area == 'UNASSIGNED') {
                ...
                pageController.animateToPage(1, duration: const Duration(milliseconds: 500), curve: Curves.linear);
                _areaBloc.state.customers![_areaBloc.state.selectedCustomerIndex!].containers!.removeAt(_areaBloc.state.selectedContainerIndex!);
              } else {
                pageController.animateToPage(1, duration: const Duration(milliseconds: 500), curve: Curves.linear);
                _areaBloc.state.customers![_areaBloc.state.selectedCustomerIndex!].containers![_areaBloc.state.selectedContainerIndex!].lotNo =
                    state.selectedDropdownLot;
              }
            }
          },
          child: BlocConsumer<AreaBloc, AreaState>(
            listener: (context, state) {
              if (state.getAvailableLotsStatus == AvailableLotsStatus.success) {
                _containerInteractionBloc.state.selectedDropdownLot = state.availableLots!.first;
              }
            },
            builder: (context, state) {
              // Debug logs to identify the issue
              print('Customers length: ${state.customers?.length}');
              print('Selected Customer Index: ${state.selectedCustomerIndex}');
              print('Selected Container Index: ${state.selectedContainerIndex}');

              bool isEnabled = state.getAreaStatus == AreaStatus.loading || state.getSearchContainerStatus == SearchContainerStatus.loading;
              bool isSkeletonEnabled = state.getAvailableLotsStatus != AvailableLotsStatus.success;
              bool noDataFound = state.getSearchContainerStatus == SearchContainerStatus.noDataFound;

              if (isEnabled) {
                return const Expanded(child: Center(child: CircularProgressIndicator()));
              }
              if (noDataFound || state.customers == null || state.customers!.isEmpty) {
                return const Expanded(child: Center(child: Text('no data found')));
              }

              // Validate indices
              int customerIndex = state.selectedCustomerIndex ?? 0;
              if (customerIndex >= state.customers!.length) customerIndex = 0;
              int containerIndex = state.selectedContainerIndex ?? 0;
              if (state.customers![customerIndex].containers == null || state.customers![customerIndex].containers!.isEmpty) {
                containerIndex = 0;
              } else if (containerIndex >= state.customers![customerIndex].containers!.length) {
                containerIndex = 0;
              }
                          ...
                          children: [
                            ...
                            Expanded(
                              child: CustomExpansionTile(
                                ...
                                itemCount: state.customers![customerIndex].containers!.length,
                                onExpand: (index) {
                                  ...
                                  return Row(
                                    children: [
                                      ...
                                      Expanded(
                                        child: InkWell(
                                          onTap: () {
                                            _containerInteractionBloc.state.getAddContainerStatus = AddContainerStatus.initial;
                                            _areaBloc.add(GetAvailableLots(
                                              area: widget.area == 'UNASSIGNED' ? state.customers![customerIndex].containers![index].toArea! : widget.area,
                                              containerNbr: state.customers![customerIndex].containers![index].containerNbr!,
                                            ));
                      ...
                      SizedBox(
                        ...
                        child: Column(
                          children: [
                            ...
                            Container(
                              height: widget.area == 'UNASSIGNED' ? lsize.maxHeight * 0.56 : lsize.maxHeight * 0.42,
                              width: lsize.maxWidth * 0.95,
                              padding: EdgeInsets.all(lsize.maxHeight * 0.02),
                              decoration: BoxDecoration(
                                color: const Color.fromRGBO(195, 169, 219, 1),
                                borderRadius: BorderRadius.circular(15),
                              ...
                              child: Column(
                                ...
                                children: [
                                    ...
                                    child: IntrinsicWidth(
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          Expanded(
                                            child: Text(
                                              state.customers![customerIndex].containers![containerIndex].containerNbr!,
                                              textAlign: TextAlign.center,
                                              style: const TextStyle(
                                                color: Color.fromRGBO(111, 54, 167, 1),
                                                fontSize: 18,
                                  ...
                                  if (widget.area == 'UNASSIGNED')
                                    Padding(
                                      padding: EdgeInsets.only(
                                        left: lsize.maxWidth * 0.01,
                                        top: lsize.maxHeight * 0.02,
                                        bottom: lsize.maxHeight * 0.02,
                                      ),
                                      child: const Text(
                                        'Area',
                                  ...
                                  if (widget.area == 'UNASSIGNED')
                                    Container(
                                      ...
                                      decoration: BoxDecoration(
                                        ...
                                        border: Border.all(color: const Color.fromRGBO(111, 54, 167, 1)),
                                        color: const Color.fromRGBO(242, 228, 255, 1),
                                      ),
                                      padding: EdgeInsets.only(left: lsize.maxWidth * 0.0415),
                                      alignment: Alignment.centerLeft,
                                      child: Text(
                                        state.customers![customerIndex].containers![containerIndex].toArea!,
                                  ...
                                  Skeletonizer(
                                    ...
                                    child: LotDropdown(
                                      selectedLot: isSkeletonEnabled ? 'lot number' : state.availableLots!.first,
                                      suggestionsCallback: (pattern) {
                                        return state.availableLots!
                                            .where(
                                      ...
                                      onPressed: () {
                                        if (_containerInteractionBloc.state.selectedDropdownLot != null) {
                                          _containerInteractionBloc.add(RelocateContainer(
                                            ...
                                            currentArea: _containerInteractionBloc.state.selectedAreaName!.name.toUpperCase(),
                                          ));
                                        } else {
                                          Customs.CMSFlushbar(size, context,
                                              icon: const Icon(Icons.error_outline_rounded), message: 'Please select Lot Number');
                                        }
                                      },
                                      style: ElevatedButton.styleFrom(
                                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                                        foregroundColor: Colors.white,
                                        backgroundColor: const Color.fromRGBO(121, 65, 177, 1),
                                      ),
                                      child: Text(
                                        widget.area == 'UNASSIGNED' ? 'Allocate' : 'Relocate',
                                      ),
                                    ),
                                  ),
                                ],
